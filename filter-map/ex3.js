...
let getStudentsPerCurriculum = (campuses, curriculumName) => {
  return campuses.filter(campus => {
    for (let index = 0; index < campus.curriculums.length; index++) {
      if(campus.curriculums[index].name === curriculumName) {
        return campus;
        // return ({campuses.city : campus.curriculums[index].numStudents});
      }
    }
  });
};
